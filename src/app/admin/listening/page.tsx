...
export default function ListeningManagement() {
  ...
  const fetchListeningItems = async () => {
    setDataLoading(true);
    try {
      const q = query(collection(db, 'listening'), orderBy('createdAt', 'desc'));
  ...
  const handleDeleteItem = async (itemId: string) => {
    if (!confirm('Are you sure you want to delete this listening practice?')) return;

    try {
      await deleteDoc(doc(db, 'listening', itemId));
      setListeningItems(listeningItems.filter(item => item.id !== itemId));
      setMessage('✅ Listening practice deleted successfully!');
...
"Listening 2","https://example.com/audio2.mp3","Another transcript","Q1|Q2|Q3",easy,A2`}
      ...
      {/* Manage Listening Items */}
      <div className="mt-6 bg-white dark:bg-slate-800 rounded-lg p-6 shadow">
        <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">📋 Manage Listening Practices</h2>

        <div className="mb-4 text-sm text-slate-600 dark:text-slate-400">
          Total Practices: <span className="font-bold text-slate-900 dark:text-white">{listeningItems.length}</span>
        </div>

        {dataLoading ? (
          <div className="text-center py-8 text-slate-600 dark:text-slate-400">Loading listening practices...</div>
        ) : listeningItems.length === 0 ? (
          <div className="text-center py-8 text-slate-600 dark:text-slate-400">No listening practices yet. Add some using the form above or upload a CSV file.</div>
        ) : (
              ...
              <div key={item.id} className="border border-slate-300 dark:border-slate-600 rounded-lg p-4 bg-slate-50 dark:bg-slate-700/30 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h3 className="text-slate-900 dark:text-white font-medium">{item.title || 'Listening Practice'}</h3>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Duration: {item.timeLimit}s</p>
                  </div>
                  <div className="flex gap-2">
