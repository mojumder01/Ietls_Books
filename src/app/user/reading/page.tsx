...
import { Reading as ReadingType } from '@/types';

export default function Reading() {
  const [passages, setPassages] = useState<ReadingType[]>([]);
  const [selectedPassage, setSelectedPassage] = useState<ReadingType | null>(null);
  ...
  useEffect(() => {
    const fetchPassages = async () => {
      try {
        const snapshot = await getDocs(collection(db, 'reading'));
  ...
  const startPractice = (passage: ReadingType) => {
  ...
  const submitAnswers = () => {
    ...
    selectedPassage.questions?.forEach((question, index) => {
  ...
  if (loading) {
    return <div className="p-6">Loading reading passages...</div>;
  }

  if (selectedPassage && !submitted) {
    return (
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">📚 Reading Practice</h1>
          <div
  ...
  if (selectedPassage && submitted) {
    ...
    const correctAnswers = Object.entries(userAnswers).filter(([index, answer]) => {
      const question = selectedPassage.questions?.[parseInt(index)];
      return (
        question &&
    ...
    return (
      <div className="p-6">
        ...
        <div className="flex gap-4">
          ...
          <button
            onClick={() => startPractice(selectedPassage)}
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-6">📚 Reading Practice</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {passages.map((passage) => (
          <div key={passage.id} className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow hover:shadow-lg transition">
            <div className="flex justify-between items-start mb-3">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Reading Passage</h3>
            ...
            <div className="mb-4 space-y-2">
              <p className="text-xs text-slate-600 dark:text-slate-400">
                ⏱️ Time Limit: {passage.timeLimit} minutes
              </p>
              <p className="text-xs text-slate-600 dark:text-slate-400">
      ...
      {passages.length === 0 && (
        <div className="text-center py-12">
          <p className="text-slate-600 dark:text-slate-400">No reading passages available yet</p>
        </div>
      )}
    </div>
  );
